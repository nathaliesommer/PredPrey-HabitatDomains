...
---
---
title: "Disentangling the role of predator hunting mode, spatial domain size, and habitat complexity: An agent-based examination of consumptive and nonconsumptive effects"
author: "Orrick et al"
date: "Nov 21 2021"
output: html_document
---
...
This document is for predator-prey interactions over 5 years.
...
FILE_TO_READ <- "../Data/YEAR5_MODELS_8x12_death_Space_Time_Habitat YEAR5_MODELS-tableNov29.csv"
...
NAME_OF_CLEAN_DATA <- "../Data/NCvsC_5year_TSH_Nov29.csv"
...
mutate(SumTime = apply(data[,c(16:39)], 1, sum)) %>% # proportion of time spent in predator free hrs 12:00-23:00
...
gg.summary.s<- s %>% 
  group_by(Pred.Strat, Pred.Start.Con, Prey.Start.Con, variable) %>%
  summarize(mean_years = round(mean(ticks)/365/24, 2), mean_i = mean(interactions), nearby = mean(nearby), scount = n())
...
sur_plot<- survivors_p + geom_text(data = gg.summary.s, aes(x = factor(Pred.Strat), y = .35, label = format(round(mean_i), digits = 2), color = "blue"), check_overlap= TRUE, show.legend = FALSE) +  geom_text(data = gg.summary.s, aes(x = factor(Pred.Strat), y = .1, label = format(round(nearby, digits = 2)), color = "red"), check_overlap= TRUE, show.legend = FALSE) + geom_text(data = gg.summary.s, aes(x = factor(Pred.Strat), y = .2, label = format(scount)), check_overlap= TRUE, show.legend = FALSE) +
     labs(caption = "Survivors, five years\nred text: interactions\n black: number of survivors \n blue text: nearby", size = 10) 
sur_plot
...
null <- read_csv("../Data/NCvsC_NULL_5year_TSH_Nov29.csv") 
...
## All simulations
```{r plotting all - NULL, echo=FALSE, fig.height=11, fig.width=10}
...
```{r prep plot with only survivors - NULL, echo=FALSE}
...
gg.summary.s.n<- s.n %>% 
  group_by(Pred.Strat, Pred.Start.Con, Prey.Start.Con, variable) %>%
  summarize(scount = n(), mean_i = mean(interactions), nearby = mean(nearby))
...
```{r Plot for survivors  of the null, echo=FALSE, fig.height=11, fig.width=10}
...
survivors.n.p.final<- survivors_n.p + geom_text(data = gg.summary.s.n, aes(x = factor(Pred.Strat), y = .35, label = format(round(mean_i), digits = 2), color = "blue"), check_overlap= TRUE, show.legend = FALSE) +  geom_text(data = gg.summary.s.n, aes(x = factor(Pred.Strat), y = .1, label = format(round(nearby, digits = 2)), color = "red"), check_overlap= TRUE, show.legend = FALSE) +
     labs(caption = "Null survivors, five years\nred text: interactions\n black: time of death (yrs) \n blue text: nearby", size = 10) 
...
```{r null vs model, echo=FALSE, fig.height=11, fig.width=10}
...
tot_sims_and_null <- p_tot +  geom_text(data = gg.summary, aes(x = factor(Pred.Strat), y = .1, label = paste0("s[shifts]: ", format(mean_years))), check_overlap= TRUE, parse = TRUE)  + geom_text(data = gg.summary.n, aes(x = factor(Pred.Strat), y = .15, label = paste0("s[null]: ", format(mean_years))), check_overlap= TRUE, color = "brown", parse = TRUE)  + 
  geom_text(data = gg.summary, aes(x = factor(Pred.Strat), y = .22, label = paste0("i: ", format(mean_i)), color = "blue"), check_overlap= TRUE, show.legend = FALSE) + geom_text(data = gg.summary, aes(x = factor(Pred.Strat), y = .27, label = paste0("nearby: ", format(nearby)), color = "red"), check_overlap= TRUE, show.legend = FALSE) + labs(caption = "All simulations, five years \nred text: interactions\n black/brown: length of survival (yrs) \n blue text: nearby", size = 10)
...
```{r t-test five year null vs total}

t.testResults_fiveyears = data.frame()
...
print(i)
  ...
  t.testResults_fiveyears<- rbind(t.testResults_fiveyears, Table_prep)
}
thetest
write.csv(t.testResults_fiveyears, "T-testResults_FiveYears_Dec20.csv")
...
## Null vs simulations for only survivors
We can also look at only the survivors

```{r null vs model survivors, echo=FALSE, fig.width=10,fig.height=11}

tot_sims_and_null_s <- survivors_p +  geom_text(data = gg.summary.s, aes(x = factor(Pred.Strat), y = .1, label = paste0("s[shifts]: ", format(scount))), check_overlap= TRUE, parse = TRUE)  + geom_text(data = gg.summary.s.n, aes(x = factor(Pred.Strat), y = .15, label = paste0("s[null]: ", format(scount))), check_overlap= TRUE, color = "brown", parse = TRUE)  + 
