---
title: "Space_Time_Habitat Null Model"
...
Overall:
Null model isn't running perfectly yet. Still has a bit of a time shift, therefore we need to go back and run it again.


First install any packages that are necessary. The order of the packages which are loaded are important.

```{r}
#Import Data

#invisible(lapply(paste0("package:", names(sessionInfo()$otherPkgs)), detach,character.only = TRUE, unload = TRUE))# Unload add-on packages
...
data <- read.csv("Data/YEAR1_NULL_8x12_death_Space_Time_Habitat_Nov19.csv", skip = 6, stringsAsFactors = FALSE, col.names=c("run Number", "Pred-Strat", "Prey-Start-Con",  "Pred-Start-Con", "Detect-Black",  "Detect-White", "Step", "ticks", "nearby", "mapAware-table", "Hour-Map",  "Black-Table", "White-Table", "DomainOverlap", "Domain-Prey"))
...
rm(list=ls()[ls() %in% c("hour0s", "hour1s", "hour2s","hour3s","hour4s","hour5s", "hour6s", "hour7s", "hour8s","hour9s","hour10s","hour11s", "hour12s","hour13s","hour14s","hour15s", "hour16s", "hour17s", "hour18s","hour19s","hour20s","hour21s", "hour22s","hour23s","transposed", "shortened", "shortened1", "shortened2", "finished", "finished2", "list_of_list", "newdata", "newdata2")])
...
Now save the data. Make sure it's the right name:
```{r}
write.csv(final2, "Data/NCvsC_Nullyear_TSH_Nov19.csv", row.names = FALSE)

```

You can now read in this file again, or keep working with 'final2'. You will need to change it's name to 'data' to keep working with it. Make sure that your read_csv is the same as above.

```{r}
getwd()
#Import Data 
data <- read_csv("Data/NCvsC_Nullyear_TSH_Nov19.csv") 

## OR just change it's name
#data<-final2

```

Here are the pretty figures. If you are using this for the fist time, sometimes its good 
```{r}
...
Figure X. Space, habitat and time shift of the three hunting strategies for each combination of small and large habitat domains for predators and prey of ONLY THE SURIVIORS.
```{r Plot}
