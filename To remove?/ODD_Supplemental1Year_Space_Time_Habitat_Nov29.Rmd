---
title: "1-Year Supplemental: Disentangling the role of predator hunting mode, spatial domain size, and habitat complexity: An agent-based examination of consumptive and nonconsumptive effects""
...
for (i in uniquegroups) {
  ...
  the_null_subset <- null %>%
    filter(Strat.Pred.Prey == i)
thetest<- t.test(the_subset$interactions, the_null_subset$interactions) 
print(i)
  print (thetest)
  Table_prep<- tidy(thetest) %>%
  mutate(category = paste0(i))
...
write.csv(t.testResults, "T-testResults_interactions_OneYear_Dec22.csv")


the_subset<- data %>%
    filter(Strat.Pred.Prey == "Active.Large.Small")
  the_null_subset <- data %>%
    filter(Strat.Pred.Prey == "Active.Large.Large")
Active<- t.test(the_subset$interactions, the_null_subset$interactions) 


the_subset<- data %>%
    filter(Strat.Pred.Prey == "Sit-and-Wait.Large.Small")
  the_null_subset <- data %>%
    filter(Strat.Pred.Prey == "Sit-and-Wait.Large.Large")
Sit.and.Wait<-t.test(the_subset$interactions, the_null_subset$interactions) 



the_subset<- data %>%
    filter(Strat.Pred.Prey == "Sit-and-Pursue.Large.Small")
  the_null_subset <- data %>%
    filter(Strat.Pred.Prey == "Sit-and-Pursue.Large.Large")
Sit.and.Pursue<-t.test(the_subset$interactions, the_null_subset$interactions) 
...
unknown<- data.frame()
Table_prep<- tidy(Sit.and.Pursue) %>%
  mutate(category = "Sit.and.Pursue")
  

unknown<- rbind(unknown, Table_prep)

write.csv(unknown, "T-testResults_interactions_OneYear_Dec22.csv")
